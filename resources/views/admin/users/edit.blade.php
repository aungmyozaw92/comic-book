...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ url('admin/users/'.$user->id) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('user_name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('user_name') ? ' is-invalid' : '' }}" name="user_name" id="input-name" type="text" placeholder="{{ __('User Name') }}" value="{{ old('user_name', $user->user_name) }}"  aria-required="true"/>
